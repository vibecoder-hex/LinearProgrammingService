    ...
    public class SimplexProcessor
    {
        ...
        public SimplexProcessor(double[][] constraints, double[] objective, double additionalVariable)
        {
            ...
            for (int i = 0; i <= constraintRows; i++)
                _simplexTable[i] = new SimplexTableObject[constraintCols + constraintRows + 1];

            for (int i = 0; i < constraintRows; i++)
            {
                for (int j = 0; j < constraintCols; j++)
                    _simplexTable[i][j].UpperBound = constraints[i][j];
                    

                for (int j = 0; j < constraintRows; j++)
                    _simplexTable[i][constraintCols + j].UpperBound = (i == j) ? additionalVariable : 0.0;

                _simplexTable[i][constraintCols + constraintRows].UpperBound = constraints[i][constraintCols];
            }
            ...
            for (int j = 0; j < constraintRows; j++)
                _simplexTable[_objectiveRow][constraintCols + j].UpperBound = 0.0;
            
            _simplexTable[_objectiveRow][constraintCols + constraintRows].UpperBound = 0.0;
